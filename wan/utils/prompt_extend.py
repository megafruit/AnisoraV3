...
class QwenPromptExpander(PromptExpander):
    ...
    def extend_with_img(self, 
                    ...
                    image: Union[Image.Image, str] = None, 
                    seed: int = -1, 
                    image_weight: float = 0.6,   # 0=不用图；(0,1)=降采样弱化；>=1=重复图像
                    text_emphasis: float = 2.0,  # >1=重复文本；(0,1)=截断文本；=1=原样
                    *args, **kwargs):
        """
        强化版：支持“先合并扩写，再按原时间骨架输出”。
        - 输入可含：At the beginning(...) ... Then(...) ... @@img&&idx
        - 会合并两段场景进行统一扩写，但要求模型用 <SEG1>/<SEG2> 分段返回；
        - 再拼回：At the beginning(...) 扩写1。Then(...) 扩写2。
        """
        import re, json, math
        try:
        ...
        def _strip_ref(s: str) -> str:
            # 去掉形如 @@path/to.png&&0 及其后缀
            return re.split(r"\s*@@[^\s]+&&\d+\s*$", s.strip(), maxsplit=1)[0].strip()
    
        def _extract_timed_segments(s: str):
            """
            尝试解析两段时间模板：
            At the beginning ( ... ), <seg1> . Then ( ... ), <seg2>
            返回： (prefix1, seg1, prefix2, seg2) ; 解析失败时返回 None
            """
            s = s.strip()
            # 宽松匹配：允许可选逗号和空格；捕获两个 prefix 与两段主体
            pat = re.compile(
                r"^(?P<prefix1>At the beginning\s*\([^)]*\)\s*,?\s*)(?P<seg1>.+?)\s*"
                r"(?P<prefix2>In the same shot\s*\([^)]*\)\s*,?\s*)(?P<seg2>.+?)\s*$",
            ...
            if not m:
                return None
            g = m.groupdict()
            return g["prefix1"], g["seg1"].strip(), g["prefix2"], g["seg2"].strip()
    
        def _soft_join_for_joint_expansion(seg1: str, seg2: str) -> str:
            """
            合并两段用于“联合理解”的输入，不做复杂指代改写，保持语义中立。
            """
            # 去掉首尾句号，避免双重标点
            s1 = seg1.strip().rstrip(" .")
            s2 = seg2.strip().rstrip(" .")
            return f"{s1}. {s2}."
    
        def _parse_tagged_output(txt: str):
            """
            从模型输出中抓取 <SEG1>...</SEG1> 与 <SEG2>...</SEG2>。
            若缺失或空，则返回 None。
            """
            m1 = re.search(r"<SEG1>\s*(.*?)\s*</SEG1>", txt, flags=re.DOTALL|re.IGNORECASE)
            m2 = re.search(r"<SEG2>\s*(.*?)\s*</SEG2>", txt, flags=re.DOTALL|re.IGNORECASE)
            if not (m1 and m2):
                return None
            seg1 = m1.group(1).strip()
            seg2 = m2.group(1).strip()
            if not seg1 or not seg2:
                return None
            return seg1, seg2
    
        # ---------- 1) 预处理输入 ----------
        raw_prompt = _strip_ref(prompt)
    
        timed = _extract_timed_segments(raw_prompt)
        # 如果没有匹配到时间骨架，则走“整体扩写原样返回”的保底逻辑
        have_timed = timed is not None
    
        if have_timed:
            prefix1, seg1, prefix2, seg2 = timed
            joint_for_expansion = _soft_join_for_joint_expansion(seg1, seg2)
        else:
            # 也可能用户已改成你说的“第二种输入形态”（无时间骨架）
            # 此时 joint_for_expansion 就是整个（剥离参考图后的）prompt
            joint_for_expansion = raw_prompt
    
        # ---------- 2) 文本“软权重” ----------
        self.model = self.model.to(self.device)
        if text_emphasis > 1:
            repeat_times = int(math.ceil(text_emphasis))
            prompt_eff = (" " + joint_for_expansion) * repeat_times
        elif 0 < text_emphasis < 1:
            keep = max(1, int(len(joint_for_expansion) * float(text_emphasis)))
            prompt_eff = joint_for_expansion[:keep]
        else:
            prompt_eff = joint_for_expansion
    
        # ---------- 3) 图像“软权重” ----------
        image_elems = []
        def _downscale_if_pil(pil_img, scale: float):
            try:
                if _PILImage is None or not isinstance(pil_img, _PILImage.Image):
                    return pil_img
                if scale >= 0.999:
                    return pil_img
                w, h = pil_img.size
                nw = max(256, int(w * scale))
                nh = max(256, int(h * scale))
                if nw < w or nh < h:
                    return pil_img.resize((nw, nh), _PILImage.BICUBIC)
                return pil_img
            except Exception:
                return pil_img
        ...
        if image is not None:
            if image_weight <= 0:
                image_elems = []
            elif 0 < image_weight < 1:
                scaled_image = _downscale_if_pil(image, float(image_weight))
                image_elems = [{"type": "image", "image": scaled_image}]
                system_prompt = (system_prompt + 
                    "\n[If image and text conflict, STRICTLY follow TEXT. Use the image loosely.]").strip()
            else:
                reps = max(1, int(math.floor(float(image_weight))))
                image_elems = [{"type": "image", "image": image}] * reps
    
        # ---------- 4) 组装消息 ----------
        # 关键：要求模型“联合理解，但严格按标签分段输出”，避免它各写各的风格漂移。
        enforce_format = (
            "\nYou are expanding two sequential scenes from ONE continuous situation."
            "\nTemporal rules (hard constraints):"
            "\n1) <SEG1> must ONLY describe content within the FIRST time window anchored by [SEG1_ANCHOR]."
            "\n   Do NOT foreshadow, anticipate, or narrate any actions or outcomes that belong to [SEG2_ANCHOR]."
            "\n2) <SEG2> must ONLY describe content within the LATER time window anchored by [SEG2_ANCHOR]."
            "\n3) Keep entities/styles consistent across both segments without introducing new objects unless strictly necessary."
            "\n4) Length cap: each segment = 1–2 sentences; be faithful and concise."
            "\nOutput EXACTLY in this format (no extra lines/JSON):"
            "\n<SEG1> ...expanded text for the first (early) scene only... </SEG1>"
            "\n<SEG2> ...expanded text for the second (later) scene only... </SEG2>"
        )
    
        sys_txt = (system_prompt + enforce_format).strip()
    
        # 为了让模型知道两段分别是什么，但不让它“分别扩写、割裂语义”，
        # 我们把 joint 段当“语境”，并把原 seg1/seg2 当“锚点说明”附在用户文本里。
        if have_timed:
            user_text = (
                "Context (read as ONE scene for coherence):\n"
                f"{prompt_eff}\n\n"
                "Anchors to expand (keep names/objects consistent across both):\n"
                f"[SEG1_ANCHOR] {seg1}\n"
                f"[SEG2_ANCHOR] {seg2}\n"
                "Time windows:\n"
                "- SEG1 covers ONLY the early window described by [SEG1_ANCHOR].\n"
                "- SEG2 covers ONLY the later window described by [SEG2_ANCHOR].\n"
                "Forbidden in SEG1: do NOT narrate events/actions that appear in [SEG2_ANCHOR].\n"
            )
        else:
            user_text = f"{prompt_eff}"
    
        messages = [
            {"role": "system", "content": [{"type": "text", "text": sys_txt}]},
            {"role": "user", "content": image_elems + [{"type": "text", "text": user_text}]},
        ]
        ...
        out_trim = [out[len(inp):] for inp, out in zip(inputs.input_ids, generated_ids)]
        expanded_raw = self.processor.batch_decode(
            out_trim, skip_special_tokens=True, clean_up_tokenization_spaces=False
        )[0].strip()
    
        # ---------- 6) 解析与回填 ----------
        seg1_out, seg2_out = None, None
        if have_timed:
            parsed = _parse_tagged_output(expanded_raw)
            if parsed:
                seg1_out, seg2_out = parsed
    
            # 回退策略：
            #  - 如果只拿到了整体扩写（或标签缺失），则把整体当“联合扩写”，
            #    尝试按句号切两半；再不行就两段都用同一扩写（至少格式正确）。
            if not seg1_out or not seg2_out:
                # 简单句切：按句号分割
                sents = re.split(r"(?<=[.!?])\s+", expanded_raw)
                if len(sents) >= 2:
                    mid = max(1, len(sents)//2)
                    seg1_out = " ".join(sents[:mid]).strip()
                    seg2_out = " ".join(sents[mid:]).strip()
                else:
                    seg1_out = expanded_raw
                    seg2_out = expanded_raw
    
            # 拼回原格式
            final_text = f"{prefix1}{seg1_out} {prefix2}{seg2_out}"
        else:
            # 无时间骨架输入：直接返回联合扩写（保持你现有行为）
            final_text = expanded_raw
        ...
        return PromptOutput(
            ...
            prompt=final_text,
            seed=seed,
            system_prompt=system_prompt,   # 返回原 system_prompt（不含 enforce_format），便于日志整洁
            message=json.dumps({"content": final_text}, ensure_ascii=False),
        )
